...
def load_data(path, feature_num=5):
    '''导入数据
...
def euclidean_dist(pointA, pointB):
    ...
    distance = math.sqrt((pointA[1] - pointB[1])**2 + (pointA[2] - pointB[2])**2)
    return distance  # 欧式距离
...
if __name__ == "__main__":
    ...
    #保存对应数据类别
    sub=np.mat(cluster)
    get_data_sub("data_sub.csv",data,sub)
